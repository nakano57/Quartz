...
update: ## Update Quartz to the latest version on Github
	go install github.com/nakano57/hugo-obsidian@latest
	@git remote show upstream || (echo "remote 'upstream' not present, setting 'upstream'" && git remote add upstream https://github.com/nakano57/quartz.git)
	git fetch upstream
...
docker: ## Serve locally using Docker
	docker run -it --volume=$(shell pwd):/quartz -p 1313:1313 ghcr.io/nakano57/quartz:hugo
